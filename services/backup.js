...
for (const backupItem of backupsArray) {
  try {
    console.log(`Attempting to backup: ${backupItem.type || 'unknown'} session ${backupItem.session.id}`);
    
    // Skip if attempted too many times
    if (backupItem.retryCount >= 3) {
      console.log(`Skipping backup: Too many retries`);
      failCount++;
      continue;
    }
    ...
    if (backupItem.type === 'checklist') {
      console.log('Processing checklist-specific backup');
      // Use the specialized function for checklist backups
      result = await backupToGitHub(
        [backupItem.session],
        false,
        backupItem.fileName || `Checklist_backup_${formatDateTimeForFile(new Date())}.xlsx`
      );
    } else {
      // Original scanner session backup logic
      result = await backupToGitHub(
        [backupItem.session],
        false,
        backupItem.fileName || `qr_scanner_backup_${formatDateTimeForFile(new Date())}.xlsx`
      );
    }
    
    if (result && result.success) {
      console.log(`Successfully backed up ${backupItem.type || 'unknown'} session: ${backupItem.session.id}`);
      successCount++;
...
// Helper function for background.js
export const addSessionToPendingBackups = async (session, type = 'scanner') => {
  try {
    // Get current pending backups
    const pendingBackups = JSON.parse(await AsyncStorage.getItem('pendingBackups') || '[]');
    
    // Check if this session is already in pending backups
    const existingIndex = pendingBackups.findIndex(b => b.session.id === session.id);
    
    if (existingIndex >= 0) {
      console.log(`Session ${session.id} already in pending backups queue`);
      return false;
    }
    
    // Add new session to pending backups
    pendingBackups.push({
      session,
      type,
      retryCount: 0,
    ...
    await AsyncStorage.setItem('pendingBackups', JSON.stringify(pendingBackups));
    console.log(`Added session ${session.id} to pending backups queue`);
    return true;
  } catch (error) {
    console.error('Error adding session to pending backups:', error);
    return false;
  }
};