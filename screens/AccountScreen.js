    ...
    <ScrollView style={styles.container}>
      <Card style={styles.card}>
        <Card.Content>
          ...
          <Text>Email: {userData?.email || 'Not available'}</Text>
          <Text>Role: {isAdmin ? 'Administrator' : 'User'}</Text>
          <Text>Name: {userData?.name || 'Not available'}</Text>
        </Card.Content>
      </Card>
...
<Card style={styles.card}>
  <Card.Content>
    ...
    <Text>Last credential sync: {lastSync || 'Never'}</Text>
...
const styles = StyleSheet.create({
  ...
  signOutButton: {
    backgroundColor: '#951d1e', // Using the accent color from the app theme
    marginTop: 20,
    marginBottom: 20,
  },
  versionText: {
